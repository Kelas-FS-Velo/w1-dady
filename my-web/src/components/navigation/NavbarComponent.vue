...
<template>
  <nav
    :class="[
      'fixed top-0 text-gray-400 w-full z-50 shadow bg-transparent flex items-center justify-between px-8 md:px-16 py-6 transition-all duration-300',
    ]"
  >
    ...
    <ul
      v-if="(!isScrolled || (isScrolled && isAnimationComplete)) && !isSideMenuOpen"
  ...
  >
    ...
    <div
      ...
      :class="{ 'translate-x-0': isSideMenuOpen, 'translate-x-full': !isSideMenuOpen }"
    >
      <div class="flex flex-col h-full pt-24 px-8">
        <ul class="space-y-8">
          ...
          >
            <a
              ...
              @click="closeSideMenu"
            >
              <span class="w-0 h-0.5 bg-gradient-to-r from-blue-400 to-purple-600 mr-0 transition-all duration-300 side-nav-line"></span>
              {{ item.name }}
            </a>
          </li>
        </ul>
        ...
        >
          <div class="text-gray-400 mb-6">
            ...
            <p class="mb-2">info@workerharder.com</p>
            <p>+1 (555) 123-4567</p>
          </div>
